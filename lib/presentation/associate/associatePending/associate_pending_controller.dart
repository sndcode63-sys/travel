...
class AssociatePendingController extends GetxController {
  ...
  final RxList<AssociateListModel> pendingAssociateList = <AssociateListModel>[].obs;
  ...
  Future<void> fetchPendingMeetings({bool isRefresh = false}) async {
    ...
    try {
      ...
      final newApproved = response
          .where((item) =>
      item.active?.toLowerCase() == "pending" &&
          !pendingAssociateList.any((e) => e.id == item.id))
          .toList();

      pendingAssociateList.addAll(newApproved);
