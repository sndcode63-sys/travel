...
class MeetingPendingController extends GetxController {
  ...
  final RxList<MeetingList> pendingMeeting = <MeetingList>[].obs;
  ...
  Future<void> fetchPendingMeetings({bool isRefresh = false}) async {
    ...
    try {
      ...
      item.visitStatus?.toLowerCase() == "pending" &&
          !pendingMeeting.any((e) => e.id == item.id))
          .toList();

      // Add new items
      pendingMeeting.addAll(newApproved);
